...
object Macros {
  def impl_with_implicits_enabled(c: Context) = {
    ...
    val tree1 = Apply(Select(Literal(Constant(1)), TermName("-$greater")), List(Literal(Constant(2))))
  ...
  def impl_with_implicits_disabled(c: Context) = {
    ...
    try {
      val tree2 = Apply(Select(Literal(Constant(1)), TermName("-$greater")), List(Literal(Constant(2))))
