...
//   type SearchResult
//   val SearchFailure: SearchResult
//   val DivergentSearchFailure: SearchResult
//   val AmbiguousSearchFailure: SearchResult

//   trait ImplicitSearch {
//     val tree: Tree
//     val context: Context
//     val pt: Type
//     def infoss = context.implicitss
...
//     class LocalScopeSearch extends SubSearch {

//     }

//     class SubSearch(val isLocal: Boolean, val isView: Boolean) {
//       val undetParams = context.outer.undetparams
//     }
//     def subSearch(isLocal: Boolean, isView: Boolean) = context.outer.undetparams match {
//       case Nil => new SubSearchNoUndets(isLocal, isView)
//       case ps  => new SubSearchWithUndets(isLocal, isView, ps)
//     }
//   }
// }
...
trait Implicits {
  ...
  def inferImplicit(tree: Tree, pt: Type, reportAmbiguous: Boolean, isView: Boolean, context: Context, saveAmbiguousDivergent: Boolean, pos: Position): SearchResult = {
    ...
    val result = ImplicitSearch(tree, pt, isView, implicitSearchContext, pos).bestImplicit
  ...
  def allViewsFrom(tp: Type, context: Context, tpars: List[Symbol]): List[(SearchResult, List[TypeConstraint])] = {
    ...
    val search    = ImplicitSearch(EmptyTree, functionType(List(tpSubsted), AnyClass.tpe), true, context.makeImplicit(reportAmbiguousErrors = false))
  ...
  class ImplicitViewSearch(val tree: Tree, val pt: Type, context0: Context, val pos: Position) extends Typer(context0) with ImplicitSearch {
    def isView      = true
    def undetParams = Nil
    def tvars       = Nil

    def matchesPtImpl(tp: Type, pt: Type): Boolean = normSubType(tp, pt) || {
      pt match {
        case Function1(ptarg, ptres) => matchesPtView(tp, ptarg, ptres)
        case _                       => false
      }
    }
    private def matchesPtView(tp: Type, ptarg: Type, ptres: Type): Boolean = {
      def matchesArgAndResult(tparg: Type, tpres: Type): Boolean = (ptarg weak_<:< tparg) && {
        ptres match {
          case HasMethodMatching(name, argtpes, restpe) =>
            tpres.hasMemberWhich(name, m => infer.isApplicableSafe(Nil, m.tpe, argtpes, restpe))
          case _ =>
            tpres <:< ptres
        }
      }
      def matchesType(tp: Type): Boolean = tp match {
        case MethodType(p :: _, restpe) if p.isImplicit => matchesType(restpe)
        case MethodType(p :: Nil, restpe)               => matchesArgAndResult(p.tpe, restpe)
        case ExistentialType(_, qtpe)                   => matchesType(normalize(qtpe))
        case Function1(arg1, res1)                      => matchesArgAndResult(arg1, res1)
        case _                                          => false
      }
      matchesType(tp)
    }
  ...
  class ImplicitValueSearch(val tree: Tree, val pt: Type, context0: Context, val pos: Position) extends Typer(context0) with ImplicitSearch {
    def isView           = false
    lazy val undetParams = context.outer.undetparams
    lazy val tvars       = undetParams map freshVar

    def matchesPtImpl(tp: Type, pt: Type) = normSubType(tp, pt)
  }

  object ImplicitSearch {
    def apply(tree: Tree, pt: Type, isView: Boolean, context: Context, pos0: Position = NoPosition): ImplicitSearch = {
      val pos = if (pos0 == NoPosition) tree.pos else pos0
      if (isView)
        new ImplicitViewSearch(tree, pt, context, pos)
      else
        new ImplicitValueSearch(tree, pt, context, pos)
    }
  }
   ...
   */
  trait ImplicitSearch extends Typer with ImplicitsContextErrors {
    lazy val ptInstantiated: Type = pt.instantiateTypeParams(undetParams, tvars)
    lazy val wildPt: Type         = approximate(pt)

    def undetParams: List[Symbol]   // The type parameters to instantiate
    def tvars: List[TypeVar]
    def isView: Boolean
    def tree: Tree
    def pos: Position
    def pt: Type
    // def wildPt: Type                // The expected type with all undetermined type parameters replaced with wildcards
    // def ptInstantiated: Type
    // def approximate(tp: Type): Type
    def matchesPtImpl(tp: Type, pt: Type): Boolean

    // lazy val ptInstantiated = pt.instantiateTypeParams(undetParams, tvars)
    // lazy val wildPt         = approximate(pt)
    ...
    def isScalaDoc = context.tree == EmptyTree

    case class AugmentedImplicitInfo(info: ImplicitInfo, ptChecked: Boolean, isLocal: Boolean) {
      lazy val itree  = atPos(pos.focus)( if (isView) inferredViewTree else inferredValueTree )
      lazy val itree1 = typed1(itree, EXPRmode, wildPt)
      lazy val itree2 = itree1 match {
        case Apply(fun, _) if isView => fun
        case _                       => adapt(itree1, EXPRmode, wildPt)
      }
      lazy val solvedTypeArgs = (
        solvedTypes(tvars, undetParams, undetParams map varianceInType(pt), upper = false, lubDepth(itree2.tpe :: pt :: Nil))
      )

      import infer._
      def fail(reason: String): SearchResult = failure(itree, reason)
      def failAndExplain(msg: String): Boolean = context.firstError match {
        case Some(err) => fail(msg + ": " + err.errMsg) ; true
        case _         => false
      }
      def hasMatchingSymbol: Boolean = {
        def loop(t: Tree): Boolean = (t.symbol == info.sym) || {
          t match {
      ...
      def finish(): SearchResult = {
        ...
        checkBounds(itree2, NoPrefix, NoSymbol, undetParams, solvedTypeArgs, "inferred ")
        context.firstError match {
          case Some(err) => return fail("type parameters weren't correctly instantiated outside of the implicit tree: " + err.errMsg)
          case _         =>
        }
        // filter out failures from type inference, don't want to remove them from undetParams!
        // we must be conservative in leaving type params in undetparams
        // prototype == WildcardType: want to remove all inferred Nothings
        val AdjustedTypeArgs(okParams, okArgs) = adjustTypeArgs(undetParams, tvars, solvedTypeArgs)
        ...
        if (failAndExplain("typing TypeApply reported errors for the implicit tree"))
          SearchFailure
        else {
          val result = new SearchResult(itree2, subst)
          if (Statistics.canEnable) Statistics.incCounter(foundImplicits)
          printInference(s"[success] found $result for pt $ptInstantiated")
          result
        }
      }
      ...
      // SI-4270 SI-5376 Always use an unattributed Ident for implicits in the local scope,
      // rather than an attributed Select, to detect shadowing.
      def inferredValueTree = if (isLocal && !isScalaDoc) info.makeIdent else info.makeSelect
      def inferredViewTree = pt match {
        case Function1(arg1, arg2) => typed1(adaptInferredTreeAsArg(inferredValueTree, approximate(arg1)), EXPRmode, approximate(arg2))
        case _                     => inferredValueTree
      }
      private def adaptInferredTreeAsArg(itree: Tree, argtpe: Type) = atPos(itree.pos)(Apply(itree, List(Ident("<argument>") setType argtpe)))
    }
     ...
     */
    private def matchesPt(info: ImplicitInfo): Boolean = info.isStablePrefix && matchesPt(info.depoly, wildPt)
    private def matchesPt(tree: Tree): Boolean         = matchesPt(tree.tpe, ptInstantiated)
    private def matchesPt(tp: Type, pt: Type): Boolean = {
      val start = if (Statistics.canEnable) Statistics.startTimer(matchesPtNanos) else null
      val result = matchesPtImpl(tp, pt)
...
//    assert(tree.isEmpty || tree.pos.isDefined, tree)
     ...
     */
    private def typedImplicit(augInfo: AugmentedImplicitInfo): SearchResult = {
      import augInfo._
      ...
      (context.openImplicits find { case OpenImplicit(info, tp, tree1) => !info.sym.isMacro && tree1.symbol == tree.symbol && dominates(pt, tp)}) match {
         ...
         case None =>
           try {
             ...
             val result = typedImplicit0(augInfo)
    ...
    private def typedImplicit0(augInfo: AugmentedImplicitInfo): SearchResult = {
      import augInfo._
      ...
      if (ptChecked || matchesPt(info))
        typedImplicit1(augInfo)
      else
        SearchFailure
    }

    private def typedImplicit1(augInfo: AugmentedImplicitInfo): SearchResult = {
      import augInfo._
      if (Statistics.canEnable) Statistics.incCounter(matchingImplicits)

      def runSearch(): SearchResult = {
        itree  // force
        itree1 // force
        if (failAndExplain("implicit adapt failed"))
          return SearchFailure

        if (Statistics.canEnable) Statistics.incCounter(typedImplicits)
        itree2 // force

        if (failAndExplain("hasMatchingSymbol reported error"))
          SearchFailure
        else if (isLocal && !hasMatchingSymbol)
          fail(s"candidate implicit ${info.sym.fullLocationString} is shadowed by ${itree1.symbol.fullLocationString}")
        else if (!matchesPt(itree2))
          fail(s"incompatible: ${itree2.tpe} does not match expected type $ptInstantiated")
        else
          augInfo.finish()
      }
    ...
    class LocalScopeSearch(iss: Infoss, private val shadowed: util.HashSet[Name]) extends ImplicitComputation(iss) {
      def this(iss: Infoss) = this(iss, util.HashSet[Name](512))

      override def isLocal = true
      def addInfos(infos: Infos): Unit = shadowed addEntries infos.map(_.name)
      def isShadowed(name: Name) = shadowed(name)
    }

    class CompanionScopeSearch(iss: Infoss) extends ImplicitComputation(iss) {
     ...
     */
    abstract class ImplicitComputation(iss: Infoss) {
      def isLocal: Boolean
      ...
      // Sorted list of eligible implicits, most frequent first.
      // val eligible = (
      //   iss flatMap (is => try is filter survives finally addInfos(is))
      //     sortBy (x => if (isView) -x.useCountView else -x.useCountArg)
      // )
      var invalidImplicits: List[ImplicitInfo] = Nil
      def checkValid(info: ImplicitInfo): Boolean = isValid(info) || {
        invalidImplicits ::= info
        false
      }
      // val invalidImplicits = iss.flatten filterNot isValid
      val eligible = {
      ...
      @tailrec private def rankImplicits(pending: Infos, acc: Infos): Infos = pending match {
        case Nil      => acc
        case i :: is  =>
          DivergentImplicitRecovery(typedImplicit(AugmentedImplicitInfo(i, ptChecked = true, isLocal)), i) match {
       ...
       */
      def findAll() = mapFrom(eligible)(info => typedImplicit(AugmentedImplicitInfo(info, ptChecked = false, isLocal)))
      ...
      def findBest(): SearchResult = {
        ...
        if (best.isFailure) {
          ...
          // Carefully threading between SI-2206 and SI-7486
          // else invalidImplicits filterNot (_.isCyclic) foreach { info =>
          //   def isSensibleAddendum = pt match {
          //     case Function1(_, out) => out <:< info.tpe.finalResultType
          //     case _                 => pt <:< info.tpe.finalResultType
          //   }
          //   // Don't pitch in with this theory unless it looks plausible that the
          //   // implicit would have helped
          //   setAddendum(pos, () =>
          //     if (isSensibleAddendum)
          //       s"\n Note: implicit ${info.sym} is not applicable here because it comes after the application point and it lacks an explicit result type"
          //     else ""
          //   )
          // }
        }
    ...
    def applicableInfos(iss: Infoss, isLocal: Boolean): Map[ImplicitInfo, SearchResult] = {
      ...
      val computation = newImplicitComputation(iss, isLocal)
    ...
    def newImplicitComputation(iss: Infoss, isLocal: Boolean) =
      if (isLocal) new LocalScopeSearch(iss) else new CompanionScopeSearch(iss)
    ...
    def searchImplicit(implicitInfoss: Infoss, isLocal: Boolean): SearchResult =
      ...
      else newImplicitComputation(implicitInfoss, isLocal).findBest()
    ...
    def allImplicitsPoly(tvars: List[TypeVar]): List[(SearchResult, List[TypeConstraint])] = {
      ...
      def eligibleInfos(iss: Infoss, isLocal: Boolean) = {
        val eligible = newImplicitComputation(iss, isLocal).eligible
        eligible.toList.flatMap { (ii: ImplicitInfo) =>
          ...
          val res = typedImplicit(AugmentedImplicitInfo(ii, ptChecked = false, isLocal))
