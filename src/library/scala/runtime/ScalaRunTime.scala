...
object ScalaRunTime extends ScalaRunTimeInterface {
  ...
  def isArray(x: AnyRef): Boolean                      = impl.isArray(x)
  ...
  def anyValClass[T <: AnyVal](value: T): Class[T] = impl.anyValClass(value)
  ...
  // Misc
  def Try[A](block: => A): ScalaRuntimeTry[A] = impl.Try(block)
...
<<<<<<< HEAD
  def Try[A](block: => A): ScalaRuntimeTry[A] = new ScalaRuntimeTryImpl[A](block)
