...
trait Function1[@specialized(scala.Int, scala.Long, scala.Float, scala.Double/*, scala.AnyRef*/) -T1, @specialized(scala.Unit, scala.Boolean, scala.Int, scala.Float, scala.Long, scala.Double/*, scala.AnyRef*/) +R] extends AnyRef { self =>
  ...
  if (!Count.locked) {
    Count.locked = true
    try {
      if (Count.map ne null) {
        val name = this.getClass.getName
        val count = Option(Count.map.get(name)).fold(1)(_ + 1)

        Count.map.put(name, count)
      }
    }
    finally Count.locked = false
  }
}
...
object Count {
  ...
  scala.sys addShutdownHook {
    ...
    val pairs = map.asScala.toList.filter(_._2 >= 100).sortBy(-_._2)
    println("Total Function1s: " + pairs.map(_._2).sum)
    pairs foreach {
