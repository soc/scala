...
trait ReOrdering[@specialized(Boolean, Int, Long, Float, Double) A] {
  def compare(x: A, y: A): Compared
}

object ReOrdering extends ReOrderingImplicitOps {
  type CompareFn[-A] = (A, A) => Compared

  def apply[A] = new Builder[A]
  class Builder[A] {
    def apply[B: ReOrdering](f: A => B): ReOrdering[A] = empty[A] -- f

    def ++[B: ReOrdering](f: A => B): ReOrdering[A] = empty[A] ++ f
    def --[B: ReOrdering](f: A => B): ReOrdering[A] = empty[A] -- f
  }
  def summon[A](implicit ord: ReOrdering[A]): ReOrdering[A] = ord
  ...
  def comparing[A](cmp: CompareFn[A]): ReOrdering[A] = new ReOrdering[A] { def compare(x: A, y: A) = cmp(x, y) }
  def string[A] : ReOrdering[A] = String map ("" + _)

  def join[A](p1: CompareFn[A], p2: CompareFn[A]): ReOrdering[A] = comparing[A]((x, y) => p1(x, y) || p2(x, y))
