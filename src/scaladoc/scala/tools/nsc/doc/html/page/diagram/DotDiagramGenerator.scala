...
class DotDiagramGenerator(settings: doc.Settings) extends DiagramGenerator {
  ...
  private def transform(e:scala.xml.Node): scala.xml.Node = e match {
    ...
    case Elem(prefix, "svg", attribs, scope, child @ _*) => {
      ...
      // !!! Replace with % again ...
      val elem0 = Elem(prefix, "svg", attribs, scope, true, child map(x => transform(x)) : _*)
      val elem1 = elem0.copy(attributes = MetaData.update(elem0.attributes, elem0.scope, new UnprefixedAttribute("id", "graph" + counter, Null)))
      elem1.copy(attributes = MetaData.update(elem1.attributes, elem1.scope, new UnprefixedAttribute("class", klass, Null)))
    }
    ...
    case img @ <image></image> => {
      ...
      // !!! Replace with % again ...
      val imgElem = img.asInstanceOf[Elem]
      imgElem.copy(attributes = MetaData.update(imgElem.attributes, imgElem.scope, new PrefixedAttribute("xlink", "href", pathToLib + file, Null)))
    }
    ...
    case g @ Elem(prefix, "g", attribs, scope, children @ _*) if (List("edge", "node").contains((g \ "@class").toString)) => {
      ...
      if (dotId.count(_ == '|') == 1) {
        ...
        // !!! Replace with % again ...
        val res0 = res.copy(attributes = MetaData.update(res.attributes, res.scope, new UnprefixedAttribute("id", id, Null)))
        res0.copy(attributes = MetaData.update(res0.attributes, res0.scope, new UnprefixedAttribute("class", (g \ "@class").toString + " " + klass, Null)))
      }
      else res
    }
    // remove titles
