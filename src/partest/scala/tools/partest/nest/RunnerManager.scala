...
class RunnerManager(kind: String, val fileManager: FileManager) {
  // kind match {
  //   case "specialized" =>
  // }
  import fileManager._
  // vlog(ClassPath.split(fileManager.universalClasspath).map("  " + _ + "\n").mkString("universalClasspath {\n", "", "}"))

  def runTest(testFile: File): TestState = {
    val runner = new Runner(testFile, fileManager)
    vlog(runner.stdClassPathElems.map("  " + _ + "\n").mkString("cp {\n", "", "}"))
