...
class ConsoleRunner extends DirectRunner {
  ...
  def main(argstr: String) {
    ...
    fileManager =
      ...
      else if (parsed isSet "--classpath") new ConsoleFileManager(parsed("--classpath"))
    ...
    def banner = {
      val vmInfo = "%s (build %s, %s)".format(javaVmName, javaVmVersion, javaVmInfo)
      val cpus   = Runtime.getRuntime().availableProcessors()
      val javacp = sys.props("java.class.path")
      val fm     = fileManager
      import fm._

    s"""|Scala compiler classes in:  $buildPrefix
        |Scala version is:           $versionMsg
        |Java runtime is:            $vmInfo
        |Scalac options are:         $scalacOpts
        |Java options are:           $javaOpts
        |Source directory is:        $srcDir
        |Available processors:       $cpus
        |Java Classpath:             $javacp
      """.stripMargin
    }
