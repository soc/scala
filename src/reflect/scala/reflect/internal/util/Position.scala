...
private[util] trait InternalPositionImpl {
  ...
  // Overlap, minus situations when one position wholly includes the other.
  def partiallyOverlaps(pos: Position) = overlaps(pos) && !(this includes pos) && !(pos includes this)
