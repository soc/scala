...
package object position {
  ...
  def newTreePositioner[U <: SymbolTable](u: U): Positioner[u.Tree] = new Positioner[u.Tree] {
    ...
    def reposition(entity: u.Tree, data: PosData): u.Tree = entity setPos wrappedPosData(entity, data)
    def sourcedPosition(entity: u.Tree, data: PosData)    = SourcedPosData(sourceFor(entity), data)
    def wrappedPosData(entity: u.Tree, data: PosData)     = new WrappedSourcedPosData(sourcedPosition(entity, data))
  }
}
