...
// Scopes
  ...
  trait SynchronizedScope extends Scope {
    ...
    override def rehash(sym: Symbol, newname: naming.Name) = synchronized { super.rehash(sym, newname) }
    override def unlink(e: ScopeEntry) = synchronized { super.unlink(e) }
    override def unlink(sym: Symbol) = synchronized { super.unlink(sym) }
    override def lookupAll(name: naming.Name) = synchronized { super.lookupAll(name) }
    override def lookupEntry(name: naming.Name) = synchronized { super.lookupEntry(name) }
