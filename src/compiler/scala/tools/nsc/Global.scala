...
import util.{ NoClassRep, MergedClassPath, StatisticsInfo, returning, stackTraceString, stackTraceHeadString }
...
class Global(var currentSettings: Settings, var reporter: Reporter)
  ...
  def classPath: SimpleClassPath = platform.classPath
...
// ------------ Phases -------------------------------------------}
  ...
  def invalidateClassPathEntries(paths: String*): (List[ClassSymbol], List[ClassSymbol]) = {
    ...
    classPath match {
      case cp: MergedClassPath =>
        def assoc(path: String): List[(SimpleClassPath, SimpleClassPath)] = {
          val dir = AbstractFile getDirectory path
          val canonical = dir.canonicalPath
          def matchesCanonical(e: SimpleClassPath) = e.origin match {
            case Some(opath) =>
        ...
        if (subst.nonEmpty) {
          ...
          def mkClassPath(elems: Iterable[SimpleClassPath]): SimpleClassPath =
  ...
  private def reSync(root: ClassSymbol,
             allEntries: SimpleClassPath, oldEntries: SimpleClassPath, newEntries: SimpleClassPath,
             invalidated: mutable.ListBuffer[ClassSymbol], failed: mutable.ListBuffer[ClassSymbol]) {
    ifDebug(informProgress(s"syncing $root, $oldEntries -> $newEntries"))

    val getName: SimpleClassPath => String = (_.name)
    def hasClasses(cp: SimpleClassPath) = cp.classes.nonEmpty
    ...
    def packageNames(cp: SimpleClassPath): Set[String] = cp.packages.toSet map getName
    def subPackage(cp: SimpleClassPath, name: String): SimpleClassPath =
      cp.packages find (cp1 => getName(cp1) == name) getOrElse SimpleClassPath("")
