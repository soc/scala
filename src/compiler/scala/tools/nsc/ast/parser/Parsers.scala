    ...
    def typeBounds(): TypeBoundsTree = {
      ...
      if (lo eq null) {
        if (hi eq null)
          ...
          TypeBoundsTree(emptyLowerBoundTree, hi) setPos hi.pos
      }
      else if (hi eq null)
        TypeBoundsTree(lo, emptyUpperBoundTree) setPos lo.pos
      else
        TypeBoundsTree(lo, hi) setPos wrappingPos(List(lo, hi))
    }
