...
abstract class TreeGen extends reflect.internal.TreeGen with TreeDSL {
  ...
  class MatchMatcher {
    ...
    def apply(matchExpr: Tree): Tree = matchExpr match {
      ...
      case Block(stats, matchEndDef) if stats forall hasSynthCaseSymbol =>
        // the assumption is once we encounter a case, the remainder of the block will consist of cases
        // the prologue may be empty, usually it is the valdef that stores the scrut
        val (prologue, cases) = stats span (s => !s.isInstanceOf[LabelDef])
        caseVirtualizedMatchOpt(matchExpr, prologue, cases, matchEndDef, identity)
      // optimized version of virtpatmat
      case Block(outerStats, orig@Block(stats, matchEndDef)) if stats forall hasSynthCaseSymbol =>
        val (prologue, cases) = stats span (s => !s.isInstanceOf[LabelDef])
    ...
    def dropSyntheticCatchAll(cases: List[CaseDef]): List[CaseDef] = cases filter {
