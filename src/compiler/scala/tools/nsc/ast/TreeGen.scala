...
abstract class TreeGen extends scala.reflect.internal.TreeGen with TreeDSL {
  ...
  def newForwarderDefDef(clazz: Symbol, forwarder: Symbol, staticTarget: Symbol) = atPos(forwarder.pos)(
    DefDef(forwarder, staticTarget.info match {
      case MethodType(Nil, ConstantType(c)) => Literal(c)
      case _                                =>
        val args = This(clazz) :: (forwarder.paramss.flatten map Ident)
        mkMethodCall(staticTarget, Nil, args)
    })
  )
