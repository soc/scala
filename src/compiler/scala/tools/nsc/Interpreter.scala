...
class Interpreter(val settings: Settings, out: PrintWriter) {
  ...
  private def isRedefining = scala.tools.util.JVMAgent.inst != null
  ...
  */
  ...
  private def makeClassLoader(): AbstractFileClassLoader = {
    ...
    if (isRedefining) new RedefiningClassLoader(virtualDirectory, parent)
    else new AbstractFileClassLoader(virtualDirectory, parent)
  }
  ...
  private def classesFor(path: String) = {
    val tok  = path take (path indexOf '$')
    def isOk(s: String) = s stripPrefix RESULT_OBJECT_PREFIX startsWith tok

    virtualDirectory filter (x => isOk(x.name))
  }
  private def classFileBytesForType(path: String): Array[Byte] = {
    val xs   = classesFor(path)
    val name = path split '$' last
    val ys   = xs filter (_.path endsWith name)

    if (ys.size == 1) ys.head.toByteArray
    else Array()
  }

  private def panalyze(name: Name, req: Request) = {
    val tp      = req compilerTypeOf name
    val path    = req fullPath name
    val symname = tp.typeSymbol.simpleName
    val bytes   = classFileBytesForType(symname.toString)
    val clazz   = classLoader tryToLoadClass symname.toString

    println("Given %s in %s: tp = %s, symname = %s, path = %s, %d bytes, clazz = %s".format(
      name, req, tp, symname, path, bytes.size, clazz
    ))
  }
  ...
  def handleTypeRedefinition(name: Name, old: Request, req: Request) = {
    ...
    val c1   = classLoader tryToLoadClass (old compilerTypeOf name).typeSymbol.simpleName.toString
    val c2   = classLoader tryToLoadClass (req compilerTypeOf name).typeSymbol.simpleName.toString

    for (cl1 <- c1 ; cl2 <- c2) {
      redefineClazz(cl1, cl2)
    }

    //
    // val oldtpe  = old.compilerTypeOf(name)
    // val newtpe  = req.compilerTypeOf(name)
    // val oldpath = old.fullPath(name)
    // val newpath = req.fullPath(name)
    //
    // println("oldpath = " + oldpath)
    // println("newpath = " + newpath)
    //
    // val classBytes = classFileBytesForType(req.fullPath(name))
    //
    //
    // (classLoader tryToLoadClass old.fullPath(name)) match {
    //   case Some(clazz)  =>
    //     val classBytes = classFileBytesForType(req.fullPath(name))
    //     redefineClazz(clazz, classBytes)
    //   case _ =>
  ...
  def handleTermRedefinition(name: Name, old: Request, req: Request) = {
    val oldtpe       = old.compilerTypeOf(name)
    val newtpe       = req.compilerTypeOf(name)
    val isCompatible = newtpe <:< oldtpe

    println(clean("Shadowing previous term '%s' (%s -> %s)".format(name, oldtpe, newtpe)))
    val oldpath = old.fullPath(name)
    val newpath = req.fullPath(name)

    println("Type is %scompatible".format(if (isCompatible) "" else "in"))
    // println("Path: %s -> %s".format(oldpath, newpath))

    val oldClasses = classesFor(oldpath)
    val newClasses = classesFor(newpath)
    (oldClasses ++ newClasses) foreach (x => println("%s, %s".format(x.name, x.path)))
    oldClasses foreach { old =>
      ()
    }
  }

  // (memory)/line22$object$$iw$$iw$$iw$$iw$$iw$$iw$$iw$$iw$$iw$.class
  // (memory)/line22$object$.class
  // (memory)/RequestResult$line22$object.class
  // (memory)/line22$object$$iw$$iw$.class
  // (memory)/line22$object$$iw$$iw$$iw$$iw$$iw$$iw$$iw$.class
  // (memory)/line22$object$$iw$$iw$$iw$$iw$$iw$$iw$$iw$$iw$$iw$$iw$.class
  // (memory)/line22$object$$iw$.class
  // (memory)/line22$object$$iw$$iw$$iw$$iw$$iw$$iw$.class
  // (memory)/line22$object$$iw$$iw$$iw$$iw$.class
  // (memory)/line22$object.class
  // (memory)/line22$object$$iw$$iw$$iw$$iw$$iw$.class
  // (memory)/line22$object$$iw$$iw$$iw$.class
  // (memory)/line22$object$$iw$$iw$$iw$$iw$$iw$$iw$$iw$$iw$.class
  // (memory)/RequestResult$line22$object$.class
  ...
  def redefineClazz(oldClazz: Class[_], bytes: Array[Byte]): Unit = {
    classLoader.recordClass(oldClazz.getName, bytes)
  }
  def redefineClazz(oldClazz: Class[_], newClazz: Class[_]): Unit = {
    redefineClazz(oldClazz, classLoader.getBytesForClass(newClazz.getName))
  }
