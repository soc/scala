...
trait PostErasure extends InfoTransform with TypingTransformers {
  ...
  class PostErasureTransformer(unit: CompilationUnit) extends TypingTransformer(unit) {
    override def transform(tree: Tree) = {
      ...
      /** Unintuitively, we must use the symbol name here, not the symbol itself,
       *  else we will find ourselves bound to the == or != call in the boxed class
       *  (most likely Object's) and not the underlying value's == method.
       */
