  ...
  class ErasureTransformer(unit: CompilationUnit) extends Transformer {
    ...
    private def doubleDefError(pair: SymbolPair) {
      ...
      if (!pair.isErroneous) {
        ...
        val when = if (memberTypesMatch) "" else " after erasure: " + exitingPostErasure(highType)
    ...
    private def checkNoDoubleDefs(root: Symbol) {
      ...
      while (opc.hasNext) {
        if (opc.sameTypeAfterPostErasure) {
          val pair = opc.currentPair
          if (!pair.memberTypesMatch) {
            log(pair)
            doubleDefError(pair)
          }
        }
