...
abstract class Mixin extends InfoTransform with ast.TreeDSL {
  ...
  case class Erasures(clazz: Symbol, base: Symbol, member: Symbol) {
    ...
    private def findMatching(enclosing: Symbol): Symbol = {
      val matching = member.matchingSymbol(enclosing.info, admit = BRIDGE)
    ...
    // lazy val erasedInClass       = erase(clazz, inClass)
    // lazy val erasedInBase        = erase(base, inBase)
    lazy val erasedInBaseInClass = erasedInBase.asSeenFrom(clazz.thisType, clazz)

    def sameErasures = erasedInBaseInClass =:= erasedInClass
    def info_s(root: Symbol, tpe: Type) = "%-40s    %s".format(member defStringSeenAs tpe, erasure.javaSig(root, tpe) getOrElse "")

    override def toString = s"""
      |$member as seen in $clazz and $base (U/E is (un)erased, C/B is class vs. base class)
      |  UC    ${info_s(clazz, inClass)}
      |  EC    ${info_s(clazz, erasedInClass)}
      |  UB    ${info_s(base, inBase)}
      |  EB    ${info_s(base, erasedInBase)}
      |  EB->C ${info_s(clazz, erasedInBaseInClass)}
      |""".stripMargin
  }
...
// --------- type transformation -----------------------------------------------
  ...
  //   val classMember = cloneBeforeErasure(traitMember, clazz)(_.info)
  //   cloneBeforeErasure(traitMember, clazz)(clazz.info memberInfo _)
  ...
  def cloneAndAddMixinMember(traitMember: Symbol, clazz: Symbol): List[Symbol] = {
    ...
    val clone2 = addMember(clazz, cloneBeforeErasure(traitMember, clazz, info2) setFlag BRIDGE)
    List(clone1, clone2)
  }

  def cloneAndAddForwarders(traitMember: Symbol, clazz: Symbol, implMember: Symbol) {
    cloneAndAddMixinMember(traitMember, clazz) foreach {
      case sym: TermSymbol => sym setAlias implMember
      case sym             => devWarning(s"Non-term symbol showed up as clone? $sym")
    }
  }
  ...
  def cloneBeforeErasure(traitMember: Symbol, clazz: Symbol, info: Type): Symbol = {
    ...
    // -      )
    // val clone1 = enteringErasure {
    //   val clone = traitMember cloneSymbol clazz
    //   clone updateInfo (traitMember.info cloneInfo clone)
    // }
    // val clone2 = enteringErasure {
    //   val clone = traitMember cloneSymbol clazz
    //   clone updateInfo (clazz.info memberInfo traitMember cloneInfo clone)
    // }
    // if (clone1.info =:= clone2.info) () else {
    //   println(s"${clone1.defString}  <===>  ${clone2.defString}")
    // }
    // clone1

    // enteringErasure {
    //   val clone = traitMember cloneSymbol clazz modifyInfo (info =>
    //     (clazz.thisType baseType traitMember.owner) memberInfo traitMember
    //   )
    //   clone updateInfo clone.info
    // }
    ...
    def mixinImplClassMembers(implClass: Symbol, mixinInterface: Symbol) {
      ...
      for (implMember <- implClass.info.decls ; if isForwarded(implMember)) {
        ...
        if (classMember eq NoSymbol) {
          ...
          if (isLateDeferred) {
            cloneAndAddForwarders(traitMember, clazz, implMember)
            // val forwarder    = cloneAndAddMixinMember(traitMember, clazz).asInstanceOf[TermSymbol] setAlias implMember
            // // forwarder.info
            // // clazz.info
            // // mixinInterface.info
      ...
      for (mixinMember <- implClass.info.decls) {
        if (isConcreteAccessor(mixinMember)) {
          if (isOverriddenAccessor(mixinMember, clazz.info.baseClasses))
            ...
            val mixedInAccessor = cloneAndAddFieldAccessor(mixinMember, clazz)
