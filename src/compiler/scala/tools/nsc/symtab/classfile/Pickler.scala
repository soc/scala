...
abstract class Pickler extends SubComponent {
  ...
  def runStaticAnalysis() {
    object sanalysis extends { val global: Pickler.this.global.type = Pickler.this.global } with typechecker.StaticAnalysis {

    }
    val stats = sanalysis.staticAnalysis(currentRun.units.toList)
    println("*** IN ***")
    stats.top.sortBy(-_.inRefs.size).take(20) foreach println
    println("\n*** OUT ***")
    stats.top.sortBy(-_.outRefs.size).take(20) foreach println
  }
  ...
  class PicklePhase(prev: Phase) extends StdPhase(prev) {
    override def run() {
      ...
      if (settings.lint.value) {
        analyzer.checkUnusedClosedWorld()
