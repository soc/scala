...
class BTypesFromSymbols[G <: Global](val global: G) extends BTypes {
  ...
  def implementedInterfaces(classSym: Symbol): List[Symbol] = {
    ...
    def isPlatformAnnotationClass(sym: Symbol): Boolean = {
      sym match {
        case ClassfileAnnotationClass => true
        case RuntimeAnnotationClass   => true
        case PlatformAnnotationClass  => true
        case _                        => false
      }
    }
    ...
    val classParents = {
      ...
      if (classSym.hasJavaAnnotationFlag) {
        //println(s"BTypesFromSymbols: <BEFORE> -- ($classSym) $parents")
        val result = /*JavaAnnotationClass.tpe ::*/ parents.filterNot(c => isPlatformAnnotationClass(c.typeSymbol))
        //println(s"BTypesFromSymbols: <AFTER > -- ($classSym) $result")
        result
      }
      else
        parents
