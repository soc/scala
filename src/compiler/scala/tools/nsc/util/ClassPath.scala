...
abstract class ClassPath[T] {
  ...
  case class ClassRep(binary: Opt[T], source: Opt[AbstractFile]) {
    def name: String = binary match {
      case Opt(x) => context.toBinaryName(x.asInstanceOf[T])
      case _      =>
  ...
  def findClass(name: String): Option[AnyClassRep] = optFindClass(name).toOption

  /**
   * Find a ClassRep given a class name of the form "package.subpackage.ClassName".
   * Does not support nested classes on .NET
   */
  def optFindClass(name: String): Opt[AnyClassRep] =
    splitWhere(name, _ == '.', doDropIndex = true) match {
      case Some((pkg, rest)) =>
        val rep = packages find2 (_.name == pkg) flatMap (_ optFindClass rest)
        rep match {
  ...
  private def traverse() = {
    ...
    dir foreach { f =>
      if (!f.isDirectory && validSourceFile(f.name))
        classBuf += ClassRep(Opt.None.asInstanceOf[Opt[T]], Opt(f))
    ...
    for (e <- entries; c <- e.classes) {
      ...
      if (indices contains name) {
        ...
        if (existing.binary.isEmpty && c.binary.isDefined)
          cls(idx) = existing.copy(binary = c.binary).asInstanceOf[AnyClassRep]
        if (existing.source.isEmpty && c.source.isDefined)
          cls(idx) = existing.copy(source = c.source).asInstanceOf[AnyClassRep]
      }
      else {
        indices(name) = count
        cls += c
