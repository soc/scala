...
abstract class Reifier extends States
  ...
  private[reify] def reifeeString(t: Tree): String = (
  ...
  private def reificationImpl(): Tree = {
    ...
    val result = reifee match {
      case tree: Tree =>
        reifyTrace("reifying = ")(reifeeString(tree))
        ...
        val ReifiedType(_, _, tpeSymtab, _, rtpe, tpeReificationIsConcrete) =
          `package`.reifyType(global)(typer, universe, mirror, tpe, concrete = false)
    ...
    val importantSymbols = (
      ...
      flatMap (s => Set(s, s.companionSymbol, s.moduleClass, s.linkedClassOfClass))
    )
    def shouldLeaveAlone(sym: Symbol) =
      (sym ne null) && ((symtab.syms contains sym) || importantSymbols(sym))

    resetAllAttrs(result, leaveAlone = {
      case ValDef(_, nme.UNIVERSE_SHORT | nme.MIRROR_SHORT, _, _) => true
      case tree                                                   => shouldLeaveAlone(tree.symbol)
    })
  }
  ...
  lazy val reification: Tree = {
    val untyped = (
      try reificationImpl()
        ...
        case ex @ (_: ReificationException | _: UnexpectedReificationException) =>
          throw ex
        case ex: Throwable =>
    ...
    if (reifyCopypaste) {
      reifyLog(s"""
        |=============================
        |${reifiedNodeToString(untyped)}
        |=============================""".stripMargin.trim)
    }
    else reifyTrace("reification = ")(untyped)
